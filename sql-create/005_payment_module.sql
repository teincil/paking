...
CREATE TABLE `order_refund` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `refund_no` varchar(32) NOT NULL COMMENT '退款单号（唯一，格式：REF+yyyyMMdd+12位数字）',
    `order_no` varchar(32) NOT NULL COMMENT '关联订单编号',
    `order_id` bigint NOT NULL COMMENT '订单ID',
    `user_id` bigint DEFAULT NULL COMMENT '用户ID',
    `refund_type` tinyint NOT NULL DEFAULT 1 COMMENT '退款类型：1用户主动退款 2系统自动退款 3管理员退款 4异常退款',
    `refund_amount` decimal(10,2) NOT NULL COMMENT '退款金额',
    `refund_balance` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '退还可用余额',
    `refund_gift` decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '退还赠送金额',
    `refund_reason` varchar(255) NOT NULL COMMENT '退款原因',
    `refund_evidence` varchar(512) DEFAULT NULL COMMENT '退款证据图片地址',
    `refund_account` varchar(64) DEFAULT NULL COMMENT '退款账户（支付宝/微信）',
    `transaction_no` varchar(64) DEFAULT NULL COMMENT '原支付交易号',
    `status` tinyint NOT NULL DEFAULT 0 COMMENT '状态：0待处理 1处理中 2退款成功 3退款失败 4已取消',
    `channel_response` text DEFAULT NULL COMMENT '渠道响应数据（JSON）',
    `operate_id` bigint DEFAULT NULL COMMENT '处理人ID（管理员）',
    `operate_name` varchar(64) DEFAULT NULL COMMENT '处理人姓名',
